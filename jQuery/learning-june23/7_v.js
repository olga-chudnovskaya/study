...
function makePopup(){
    ...
    $('screen').addClass('active'); // объекту screen присваевается класс active
    // document.querySelector('screen').classList('active');//то же самое, но просто на Js
}

function monthLength(month, year){
    let a = new Date(year, month); // YYY/M/D 0:00:00.000
    let b = new Date(year, month + 1); // YYY/M+1/D 0:00:00.000
    let c = Math.floor((b - a) / (1000 * 60 * 60 * 24)); // сколько дней прошло
    console.log(a);
    console.log(b);
    console.log(c);
